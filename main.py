...
import itertools
import torch
from sac.sac import  TRPO_SAC
from sac.td3 import TD3
from torch.utils.tensorboard import SummaryWriter
from sac.replay_memory import HERReplayMemory
...
parser.add_argument('--gamma', type=float, default=0.99, metavar='G',
                    help='discount factor for reward (default: 0.86)')
...
parser.add_argument('--ppo_update_interval', type=int, default=10,
                    help='How often to perform PPO updates (default: 7)')
parser.add_argument('--trpo_update_interval', type=int, default=5,
                    help='How often to perform TRPO updates (default: 7)')
parser.add_argument('--kl_threshold', type=float, default=0.01,
                    help='KL divergence threshold for TRPO (default: 0.03)')
parser.add_argument('--kl_constraint', type=float, default=0.1, help='KL divergence constraint')
parser.add_argument('--cuda', action="store_true",
                    help='run on CUDA (default: False)')
parser.add_argument('--policy_delay', type=int, default=2, metavar='N',
                    help='Delay for policy updates (default: 2)')
...
writer = SummaryWriter(
    'runs/{}_SAC_{}_{}_{}'.format(
        ...
        'experiment',args.env_name,
        args.policy, 
        args.exp_case  # Include the experiment case name here
    )
)
# Memory
...
for i_episode in itertools.count(1):
    ...
    while not done:
        ...
        # Push the transition into memory, using the current state as the goal
        memory.push(state, action, reward, next_state, mask, next_state)
